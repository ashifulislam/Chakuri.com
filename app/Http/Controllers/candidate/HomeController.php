...
public function showHome(){
    $data['categories']=JobCategory::with('jobPosts')->get();
//dd(($data['categories']['1']['jobPosts']));
    $data['jobPosts']=JobPost::where('status','approved')->get();
    $data['recentJobs']=JobPost::where('status','approved')->with(['employer','jobCategory'])->orderBy('id','DESC')->limit(2)->get();
    $data['fullTimeJobs']=JobPost::where('status','approved')->with(['employer','jobCategory'])->where('employmentStatus','FullTime')->orderBy('id','DESC')->limit(2)->get();
    $data['partTimeJobs']=JobPost::where('status','approved')->with(['employer','jobCategory'])->where('employmentStatus','PartTime')->orderBy('id','DESC')->limit(2)->get();
...
public function searchjob(Request $request){
    ...
    $data['jobPosts']=JobPost::with(['employer','jobCategory'])->Where('categoryTypeId',$request->categoryTypeId)->orWhere('location',$request->location)->orderBy('id','DESC')->get();
   // dd($data['jobPosts']);
    return view('candidate.searchResult',$data);
}
